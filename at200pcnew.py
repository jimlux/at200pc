...
# jimlux 27 June 2021 - added code to allow using an RFC-2217 serial port
#                     - added code to get port name off comnmand line
#                     - change hashbang at beginning to just python3
...
if sys.platform[0:3] == "win":
  ...
  TTY_NAME = "rfc2217://192.168.1.55:2217"	# Linux name of serial port for the AT-200PC
  TTY_NAME = "rfc2217://antbox1.local:2217"  # Linux name of serial port for the AT-200PC
  #TTY_NAME = "/dev/tty.usbserial-AG0JR13N"

if len(sys.argv) > 1:
  print("port name:%s"%sys.argv[1])
  TTY_NAME = sys.argv[1]
...
class Application(tkinter.Tk):
  ...
  """ added code here to try using "serial for url" before trying the regular open. I think,
      if the TTY_NAME is more conventional "/dev/ttyUSB0" that that version exceptions, and then
      it tries the regular open
  """
  def main(self):
    ...
    while not self.serial and self.running:
      if serial and win32file:
        try:
          try:
            self.serial = serial.serial_for_url(TTY_NAME,timeout=0.05)
